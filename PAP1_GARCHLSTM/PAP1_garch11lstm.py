...
# t = ['^DJI', '^GSPC', '^IXIC', 'GC=F', 'EURUSD=X']
FD = FinData()
prices, logreturns = FD.get_numpy_data('^GSPC', '2014-01-01', '2024-01-01')
...
class GARCHLSTMCell(nn.Module):
    ...
    def forward(self, demeaned_input_tensor, hidden_sig2_tensor, hidden_memory_tensor):
        ...
        omega, alpha, beta = self.reparameterize_garch()
        o = self.garch_model(demeaned_input_tensor, omega, alpha, beta) #(batch, hidden)
    ...
    def reparameterize_garch(self):
        raw_omega, raw_alpha, raw_beta = self._raw_omega, self._raw_alpha, self._raw_beta
        omega = torch.nn.functional.softplus(raw_omega) # => omega > 0
        alpha_unnorm = torch.exp(raw_alpha)
        beta_unnorm  = torch.exp(raw_beta)
        sum_unnorm = 1.0 + alpha_unnorm + beta_unnorm # Sum for alpha + beta < 1
    ...
    def view_params(self):
        omega, alpha, beta = self.reparameterize_garch()
        return omega.to(torch.half).item(), alpha.to(torch.half).item(), beta.to(torch.half).item()
    
    def garch_model(self, input_tensor, omega, alpha, beta):
        self.resids_t = input_tensor - mean_model(input_tensor)
        sig2_t = omega + alpha * self.resids_t**2 + beta * self.sig2_prev # (64, 1) volatilies
...
X_batch, y_batch, e_batch = next(iter(train_dataloader))
print(X_batch.shape, y_batch.shape, e_batch.shape)
